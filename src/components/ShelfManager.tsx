...
export function ShelfManager({ bookId, bookTitle, bookCoverUrl, userId, onUpdate }: ShelfManagerProps) {
    ...
    const menuItemBase =
        "flex w-full items-center justify-between rounded-xl px-3.5 py-3 text-[15px] sm:text-base font-medium transition-all duration-150 hover:bg-black/5 dark:hover:bg-white/6 active:scale-[0.98]";
    ...
    return (
        <div className="relative">
            ...
            <button
                ...
                disabled={isLoading}
                className="flex size-10 items-center justify-center rounded-full transition-all duration-150 hover:bg-black/5 dark:hover:bg-white/8 active:scale-95 disabled:opacity-50"
            ...
            {isMenuOpen && (
                <>
                    ...
                    <div className="glass-elevated absolute left-0 top-12 z-50 flex w-56 max-w-[calc(100vw-3rem)] flex-col overflow-hidden rounded-2xl p-2 shadow-2xl">
                        {[
                        ...
                        {currentStatus && (
                            <>
                                ...
                                <button
                                    ...
                                    className="flex w-full items-center gap-2.5 rounded-xl px-3.5 py-3 text-[15px] sm:text-base font-medium transition-all duration-150 text-red-600 dark:text-red-400 hover:bg-red-500/10 active:scale-[0.98]"
                                >
                                    <Trash2 size={16} />
                                    Remove from Shelf
                                </button>
            ...
            {isReviewModalOpen && (
                <div
                    className="fixed inset-0 z-60 flex items-center justify-center p-4 backdrop-blur-md bg-black/40 dark:bg-black/60"
                >
                    <div className="glass-elevated flex w-full max-w-lg max-h-[90vh] overflow-y-auto flex-col gap-5 sm:gap-6 rounded-3xl p-5 sm:p-8 shadow-2xl">
                        <div className="flex items-start justify-between">
                            ...
                            <button
                                ...
                                className="flex size-9 items-center justify-center rounded-full transition-all hover:bg-black/5 dark:hover:bg-white/8"
                        ...
                        <textarea
                            ...
                            className="h-32 sm:h-36 w-full resize-none rounded-2xl border-0 p-4 text-base leading-relaxed outline-none transition-shadow focus:ring-2 focus:ring-[var(--color-accent)]/30"
                            style={{
                        ...
                        <div className="flex gap-3">
                            <button
                                onClick={() => handleSaveReview(false)}
                                className="flex-1 rounded-full py-2.5 sm:py-3 text-base font-medium transition-all duration-150 hover:bg-black/5 dark:hover:bg-white/8 active:scale-[0.98]"
                                style={{ color: "var(--color-ink-muted)" }}
                            >
                                Skip
                            </button>
                            <button
                                ...
                                disabled={isLoading}
                                className="flex flex-1 items-center justify-center rounded-full py-2.5 sm:py-3 text-base font-medium text-white transition-all duration-150 hover:opacity-90 active:scale-[0.98] disabled:opacity-50"
                                style={{ background: "var(--color-accent)" }}
                            >
                                {isLoading ? <Loader2 size={18} className="animate-spin" /> : "Save review"}
                            </button>
                        </div>
                    </div>
                </div>
